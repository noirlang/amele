...
install -m 755 "$ROOT_DIR/target/release/amele" "$APPDIR/usr/bin/amele-forensic-tool"
...
if command -v magick >/dev/null 2>&1; then
  magick "$ROOT_DIR/ui/assets/logo/icon.png" -resize 256x256 "$APPDIR/usr/share/icons/hicolor/256x256/apps/amele.png"
elif command -v convert >/dev/null 2>&1; then
  convert "$ROOT_DIR/ui/assets/logo/icon.png" -resize 256x256 "$APPDIR/usr/share/icons/hicolor/256x256/apps/amele.png"
else
  install -m 644 "$ROOT_DIR/ui/assets/logo/icon.png" "$APPDIR/usr/share/icons/hicolor/256x256/apps/amele.png"
fi
install -m 644 "$APPDIR/usr/share/icons/hicolor/256x256/apps/amele.png" "$APPDIR/amele.png"

NO_STRIP="${NO_STRIP:-1}" STRIP="${STRIP:-/usr/bin/strip}" OUTPUT="$APPIMAGE" "$LINUXDEPLOY_RUN" \
  --appdir "$APPDIR" \
  --executable "$APPDIR/usr/bin/amele-forensic-tool" \
  --deploy-deps-only "$APPDIR/usr/lib/x86_64-linux-gnu/webkit2gtk-4.1" \
  --desktop-file "$APPDIR/amele.desktop" \
  --icon-file "$APPDIR/amele.png" \
