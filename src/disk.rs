...
pub fn disk_size(path: impl AsRef<Path>) -> AmeleResult<u64> {
    disk_size_impl(path.as_ref())
}

/// Platforma göre yerel diskleri listeler.
pub fn list_disks() -> AmeleResult<Vec<DiskInfo>> {
    list_disks_impl()
}
    ...
    C: FnMut() -> DiskAcquisitionControl,
{
    ...
    if source_size == 0 {
        let err = AmeleError::new(HataKodu::DiskBoyut, "Kaynak boyut alinamadi");
        runtime_log(
            LogLevel::Error,
            "disk",
    ...
    if let Some(parent) = task.target.parent() {
        ...
        fs::create_dir_all(parent).map_err(|err| {
            let w_err = AmeleError::io(HataKodu::DosyaYazma, "Hedef klasor olusturulamadi", err);
            runtime_log(
                LogLevel::Error,
                "disk",
    ...
    let mut source = File::open(&task.source).map_err(|err| {
        let w_err = AmeleError::io(HataKodu::DiskErisim, "Kaynak acilamadi", err);
        runtime_log(
            LogLevel::Error,
            "disk",
    ...
    if task.start_offset > 0 {
        ...
        source
            ...
            .map_err(|err| {
                let w_err = AmeleError::io(HataKodu::DiskOkuma, "Kaynak konumlanamadi", err);
    ...
    let mut target = File::create(&task.target).map_err(|err| {
        let w_err = AmeleError::io(HataKodu::DosyaYazma, "Hedef dosya olusturulamadi", err);
        runtime_log(
            LogLevel::Error,
            "disk",
    ...
    if total == 0 {
        let err = AmeleError::new(HataKodu::DiskBoyut, "Kopyalanacak veri yok");
...
fn mark_partial(path: &Path) -> AmeleResult<PathBuf> {
    ...
    if path.exists() {
        ...
        fs::rename(path, &partial).map_err(|err| {
            let w_err = AmeleError::io(HataKodu::DosyaYazma, "Partial dosya tasinamadi", err);
            runtime_log(
                LogLevel::Error,
                "disk",
...
fn disk_size_impl(path: &Path) -> AmeleResult<u64> {
    ...
    let w_err = AmeleError::new(HataKodu::DiskBoyut, "Disk boyutu alinamadi");
    runtime_log(
        LogLevel::Error,
        "disk",
...
fn disk_size_impl(path: &Path) -> AmeleResult<u64> {
    ...
    if !path.to_string_lossy().starts_with(r"\\.\PhysicalDrive") {
        return std::fs::metadata(path)
            ...
            .map_err(|err| {
                let w_err = AmeleError::io(HataKodu::DosyaAcilamadi, "Dosya boyutu alinamadi", err);
                runtime_log(
                    LogLevel::Error,
                    "disk",
    ...
    if handle == INVALID_HANDLE_VALUE {
        let w_err = AmeleError::new(HataKodu::DiskErisim, "Disk acilamadi");
        runtime_log(
            LogLevel::Error,
            "disk",
...
fn list_disks_impl() -> AmeleResult<Vec<DiskInfo>> {
    runtime_log(
        LogLevel::Info,
        "disk",
