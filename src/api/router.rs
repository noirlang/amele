...
pub fn route_api(method: &str, path: &str, body: &[u8]) -> Response {
    match (method, path) {
        ...
        ("POST", "/api/ram-list-processes-start") => ram::ram_list_processes_start_endpoint(body),
        ("POST", "/api/ram-process-details") => ram::ram_process_details_endpoint(body),
        ("POST", "/api/ram-process-details-start") => ram::ram_process_details_start_endpoint(body),
